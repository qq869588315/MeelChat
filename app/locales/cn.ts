...
const cn = {
  ...
  Settings: {
    ...
    Access: {
      ...
      CustomModel: {
        Title: "可用模型",
        SubTitle: "获取当前接口可用模型，聊天页只显示这些模型",
        Fetch: "获取模型列表",
        Fetching: "获取中...",
        FetchSuccess: (count: number) => `已获取 ${count} 个模型`,
        FetchFailed: (message: string) => `获取模型列表失败：${message}`,
        FetchEmpty: "接口已接通，但没有返回可用模型",
        EmptyHint: "请先填写接口地址和 API Key，然后获取可用模型",
        EmptySelector: "请先在设置中获取模型列表",
        ConfigureFirst: "配置模型",
        OnlyFetchedHint: (count: number) =>
          `已获取 ${count} 个模型，聊天页将仅显示这些模型`,
        ManualHint: (count: number) => `已配置 ${count} 个可用模型`,
